...
def test_control_rate_is_reported_once_per_window(monkeypatch, capsys):
    times = iter(index * 0.02 for index in range(51))
    ...
    assert output.count("[CONTROL RATE]") == 1
    assert "state=pd_brake" in output
    assert "hz=50.0" in output
    assert "late=0/50" in output
    assert "INFERENCE TIMEOUT" not in output
