...
class RobotModel:
    def __init__(
        ...
        # The SONIC controller only needs kinematics here.  Building visual and
        # collision geometry would make the headless PICO path depend on the
        # complete G1 CAD mesh set, so construct a model-only wrapper.
        if set_floating_base:
            model = pin.buildModelFromUrdf(urdf_path, pin.JointModelFreeFlyer())
        else:
            model = pin.buildModelFromUrdf(urdf_path)
        self.pinocchio_wrapper = pin.RobotWrapper(model)
