...
def state_requests_sonic(info: Any, target: str = "sonic_teleop") -> bool:
    ...
    if transition is not None:
        ...
        return isinstance(destination, dict) and destination.get("name") == target
    current = info.get("current")
    return isinstance(current, dict) and current.get("name") == target
...
class PicoPipeline:
    ...
    def start(self) -> None:
        ...
        wrist_source = env.get("WRIST_SOURCE", "pico_g1_legacy")
        manager = [
