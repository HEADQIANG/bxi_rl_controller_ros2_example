...
if [[ "${REMOTE_KEYBOARD}" == "1" || "${REMOTE_KEYBOARD}" == "true" ]]; then
  echo "[sonic-sim2sim-controller] keyboard: ! -> pd_brake, 1 -> normal, 6 -> sonic_teleop, 3 -> dance"
  ros2 run remote_controller remote_controller \
    --keyboard \
    --config "${REMOTE_CONFIG}" \
    --hot-reload true
else
  echo "[sonic-sim2sim-controller] gamepad: RB+B -> pd_brake, RB+X -> normal, RT+X -> sonic_teleop, LB+X -> dance"
  ros2 run remote_controller remote_controller \
    --config "${REMOTE_CONFIG}" \
    --hot-reload true
fi
